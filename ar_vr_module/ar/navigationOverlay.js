/**
 * Draws AR navigation overlay on a canvas.
 * @param {CanvasRenderingContext2D} ctx
 * @param {number} w - canvas width
 * @param {number} h - canvas height
 * @param {string} currentLabel - label of current step
 * @param {string} nextLabel    - label of next step
 * @param {{x:number, y:number}} current - map coords of current node
 * @param {{x:number, y:number}} next    - map coords of next node
 */
